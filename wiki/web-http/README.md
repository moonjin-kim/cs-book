...
| --- | --- | --- |
| 1 | HTTP 메시지와 Semantics | Method, URI, header, body, status code가 어떤 의미를 갖고 서버 동작을 어떻게 표현하는가? |
| 2 | URI와 REST API 설계 | URI, URL, query, path variable, resource, representation, stateless를 구분할 수 있는가? |
| 3 | HTTP 버전과 연결 관리 | HTTP/1.1, HTTP/2, HTTP/3의 연결 재사용, multiplexing, HOL blocking 차이는 무엇인가? |
| 4 | Cookie, Session, 인증 상태 | stateless HTTP 위에서 cookie/session/token으로 상태를 유지할 때의 보안·확장성 trade-off는 무엇인가? |
| 5 | HTTP Cache와 CDN | Cache-Control, ETag, Last-Modified, Vary, private/public cache를 설명할 수 있는가? |
| 6 | CORS와 브라우저 보안 경계 | Same-Origin Policy, simple/preflight/credential request, CORS header를 구분할 수 있는가? |
| 7 | Proxy, Web Server, Rendering | forward/reverse proxy, web server/WAS, SSR/CSR/SSG, PRG를 실무 흐름으로 설명할 수 있는가? |
| 8 | 실전 면접 Q&A | 짧은 답변으로 HTTP method, status, cache, cookie, CORS, proxy 질문을 빠르게 복습할 수 있는가? |
