...
| 전문 검색에 `LIKE '%..%'` 대신 Elasticsearch를 쓰는 이유는? | `LIKE '%..%'`는 앞부분이 와일드카드라 B-Tree 인덱스를 못 타 full scan이 됩니다. Elasticsearch는 텍스트를 토큰으로 쪼개 **역색인(inverted index)**(단어 → 문서 목록)을 만들어 대량 문서에서도 빠른 검색·스코어링·형태소 분석을 제공합니다. |
