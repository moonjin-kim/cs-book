...
트랜잭션·인덱스가 "무엇을 보장하는가"라면, 스토리지 엔진은 "그걸 디스크에서 어떻게 빠르고 안전하게 하는가"입니다. 핵심은 **버퍼 풀 + WAL + 순차 쓰기**입니다.

### 버퍼 풀과 페이지

DB는 행 단위가 아니라 **page**(InnoDB 16KB, PostgreSQL 8KB) 단위로 디스크와 메모리를 오갑니다. 자주 쓰는 page는 메모리의 **버퍼 풀(buffer pool)** 에 캐싱됩니다.

| 개념 | 설명 |
| --- | --- |
| Page | 읽기/쓰기의 최소 단위. 인덱스도 데이터도 page로 저장 |
| Buffer Pool | page를 캐싱하는 메모리 영역. hit율이 성능을 좌우 |
| Dirty Page | 메모리에서 수정됐지만 아직 디스크에 안 내려간 page |
| Flush | dirty page를 디스크에 반영(주로 background로 모아서) |

면접 포인트:

- 쓰기가 있어도 즉시 data 파일에 랜덤하게 쓰지 않습니다. 버퍼 풀에서 수정하고, 디스크 반영은 뒤로 미뤄 **랜덤 I/O를 줄입니다**.
- InnoDB는 단순 LRU가 아니라 **midpoint insertion LRU**를 써서, 큰 스캔 한 번이 자주 쓰는 page를 버퍼 풀에서 밀어내는 것을 막습니다.
- 버퍼 풀보다 큰 데이터를 랜덤 접근하면 hit율이 떨어져 디스크 I/O가 급증합니다. "메모리에 올라오는 working set 크기"가 성능의 실질 지표입니다.

### WAL(Write-Ahead Log)과 내구성

dirty page를 아직 안 내렸는데 서버가 죽으면? 이를 막는 것이 **WAL(Write-Ahead Logging)** 입니다. InnoDB의 redo log, PostgreSQL의 WAL이 여기에 해당합니다.

원칙: **data page보다 로그를 먼저 디스크에 쓴다.**

```text
1. 버퍼 풀에서 page 수정 (메모리)
2. 변경 내용을 redo log(WAL)에 순차 append → commit 시 fsync
3. data page는 나중에 background로 flush
```

| 로그 | 역할 |
| --- | --- |
| redo log (WAL) | 커밋됐지만 아직 data page에 반영 안 된 변경을 재적용(crash 복구) |
| undo log | 미커밋 변경 롤백 + MVCC의 이전 버전 제공 |

면접 포인트:

- WAL은 **순차 write**라 랜덤한 data page write보다 훨씬 빠릅니다. 커밋의 내구성 비용을 "작은 순차 로그 fsync"로 치환하는 것이 핵심 아이디어입니다.
- **Group commit**: 여러 트랜잭션의 커밋 fsync를 묶어 한 번에 수행해 디스크 I/O 횟수를 줄입니다.
- 로그를 먼저 안 쓰고 data를 먼저 쓰면 crash 시 절반만 반영된 상태를 복구할 수 없습니다. "로그 먼저"가 ACID의 D(내구성)를 만드는 실제 메커니즘입니다.

### Checkpoint와 Crash Recovery

로그가 무한히 쌓이면 복구가 느려지므로 주기적으로 **checkpoint**를 찍습니다.

- checkpoint: 그 시점까지의 dirty page를 디스크에 반영하고, "여기까지는 data에 안전하게 들어갔다"는 지점을 기록.
- 복구: 마지막 checkpoint 이후의 redo log만 재적용(redo)하고, 미커밋 트랜잭션은 undo로 롤백. → 전체 로그가 아니라 checkpoint 이후만 보므로 복구가 빨라짐.

면접 포인트: 오래 열린 트랜잭션은 undo 정리(purge/vacuum)를 막고, checkpoint가 밀리게 해 복구 시간과 로그 크기를 키웁니다.

### Clustered Index vs Heap

인덱스 leaf에 무엇이 저장되는지가 엔진마다 다릅니다.

| 엔진 | 저장 방식 |
| --- | --- |
| InnoDB(MySQL) | **clustered index** — PK의 B+Tree leaf에 행 전체가 저장됨. secondary index leaf에는 PK 값이 들어감 |
| PostgreSQL | **heap** — 행은 heap에 저장되고 모든 인덱스는 물리 위치(ctid)를 가리킴 |

면접 포인트:

- InnoDB에서 secondary index로 조회하면 leaf에서 PK를 얻고 **다시 clustered index를 타는 2단계 조회**가 일어납니다. 그래서 covering index(필요 컬럼을 인덱스에 포함)로 이 2차 조회를 없애는 튜닝이 중요합니다.
- InnoDB는 PK 순서로 물리 정렬되므로 **단조 증가 PK(auto_increment)** 가 유리하고, 랜덤 UUID PK는 page split과 write amplification을 유발합니다.
- PostgreSQL은 update 시 새 튜플을 heap에 추가(append)하고 옛 튜플을 dead로 남겨 **VACUUM**으로 정리합니다. HOT update로 인덱스 갱신을 줄이는 최적화가 있습니다.

### LSM-Tree vs B-Tree

관계형 DB의 기본은 **B+Tree**(읽기 최적, in-place update)지만, 쓰기가 매우 많은 시스템(Cassandra, RocksDB, HBase)은 **LSM-Tree**(Log-Structured Merge)를 씁니다.

| 구조 | 쓰기 | 읽기 |
| --- | --- | --- |
| B+Tree | data page를 제자리에서 갱신(랜덤 write) + WAL | leaf까지 O(log N), 캐시 친화적 |
| LSM-Tree | 메모리(memtable)에 순차 축적 → SSTable로 flush → compaction | 여러 레벨을 조회(bloom filter로 스킵) |

LSM 동작:

```text
write → memtable(메모리, 정렬됨) → 가득 차면 SSTable(디스크, immutable)로 flush
        → 백그라운드 compaction으로 SSTable 병합·정리
read  → memtable → 최신 SSTable부터 조회 (bloom filter로 없는 파일 스킵)
```

면접 포인트:

- LSM은 모든 쓰기를 **순차 append**로 바꿔 write throughput이 높지만, compaction이 background에서 write amplification과 I/O를 만듭니다.
- B-Tree는 읽기가 예측 가능하고 in-place라 공간 효율이 좋지만, 랜덤 write와 page split 비용이 있습니다.
- "쓰기 폭주 + 시계열/로그성 데이터"면 LSM 계열, "범용 트랜잭션 + 복잡한 조회"면 B+Tree 계열이 일반적 선택입니다.
...
| 질문 | 답변 |
| --- | --- |
| 데이터베이스(DB)가 뭔가? | 여러 데이터를 안전하게 저장하고, 필요할 때 빠르게 찾아 꺼내 쓰는 창고입니다. 여러 사람이 동시에 써도 데이터가 꼬이지 않게 관리해 줍니다. |
| 관계형 데이터베이스(RDB)가 뭔가? | 데이터를 표(table) 형태로, 즉 행과 열로 정리해서 저장하는 DB입니다. 엑셀 표를 여러 개 만들고 서로 연결해 쓴다고 생각하면 쉽습니다. |
| 테이블(table), 행(row), 열(column)은 각각 뭔가? | 테이블은 같은 종류의 데이터를 모아둔 표, 행은 그 표의 한 줄(예: 회원 한 명), 열은 각 줄이 가진 속성(예: 이름, 나이)입니다. |
| 트랜잭션이 뭔가? | 여러 작업을 "하나로 묶은 한 덩어리"입니다. 예를 들어 계좌 이체는 "내 돈 빼기 + 상대 돈 넣기"가 반드시 함께 되거나 함께 취소돼야 하는데, 이렇게 통째로 묶는 것이 트랜잭션입니다. |
| ACID가 뭔가? | 트랜잭션이 안전하게 지켜야 할 4가지 성질입니다. 다 되거나 다 취소(원자성), 규칙을 어기지 않음(일관성), 동시에 해도 서로 안 꼬임(격리성), 한번 저장하면 사고가 나도 안 사라짐(지속성). |
| 기본키(Primary Key)가 뭔가? | 한 테이블에서 각 행을 겹치지 않게 구별해 주는 대표 번호표입니다. 회원 테이블의 회원번호처럼, 이 값만 보면 어떤 행인지 딱 알 수 있습니다. |
| 외래키(Foreign Key)가 뭔가? | 다른 테이블의 데이터를 가리키는 연결 고리입니다. 예를 들어 주문 테이블이 "이 주문을 한 회원은 누구"인지 회원 테이블을 가리킬 때 씁니다. |
| 기본키와 외래키의 차이는? | 기본키는 "내 테이블 안에서 나를 구별하는 번호"이고, 외래키는 "다른 테이블의 행을 가리키는 참조"입니다. 기본키는 자기 식별용, 외래키는 관계 연결용입니다. |
| 인덱스가 뭐고 왜 쓰나? | 책 뒤의 "찾아보기(색인)"와 같습니다. 원하는 데이터를 처음부터 끝까지 다 뒤지지 않고 빠르게 찾게 해줍니다. 대신 데이터를 추가/수정할 때 색인도 같이 고쳐야 해서 쓰기가 조금 느려지고 저장 공간도 더 씁니다. |
| 정규화가 뭔가? | 같은 데이터가 여기저기 중복되지 않도록 테이블을 잘 나누는 정리 작업입니다. 중복을 줄이면 한 곳만 고쳐도 되고, 데이터가 서로 안 맞는 실수를 막을 수 있습니다. |
| 락(Lock)이 뭔가? | 여러 사람이 같은 데이터를 동시에 바꾸려 할 때 서로 충돌하지 않도록 "지금 내가 쓰는 중"이라고 잠가 두는 것입니다. 화장실 문을 잠그는 것과 비슷합니다. |
| RDB와 NoSQL은 어떻게 다른가? | RDB는 표 구조와 규칙이 엄격해 정확함이 중요한 업무에 강하고, NoSQL은 구조가 유연하고 대규모로 나눠 담기(수평 확장)에 유리합니다. 정합성이 중요하면 RDB, 대량·유연함이 중요하면 NoSQL을 흔히 씁니다. |
...
| 질문 | 답변 |
| --- | --- |
| RDB가 단순 저장소와 다른 점은? | 제약 조건(unique, foreign key, check 등)으로 데이터 정합성을 강제하는 시스템입니다. |
| 애플리케이션 검증만으로 정합성을 지키기 어려운 이유는? | 동시 요청에서 race condition을 막기 어렵기 때문에 unique key, foreign key, transaction을 함께 써야 불변식을 안정적으로 지킬 수 있습니다. |
| Surrogate key와 natural key의 트레이드오프는? | surrogate key는 변경 가능성이 낮고 join key로 단순하지만 비즈니스 중복은 unique constraint로 따로 막아야 하고, natural key는 도메인 의미가 명확하지만 정책 변경 시 key 변경 비용이 큽니다. |
| Candidate key, primary key, alternate key의 관계는? | row를 유일 식별할 수 있는 후보가 candidate key, 그중 대표로 선택한 것이 primary key, 선택되지 않은 나머지가 alternate key입니다. |
| 3NF와 BCNF의 차이는? | 3NF는 이행적 종속을 제거한 상태이고, BCNF는 모든 결정자가 후보키인 더 강한 정규형입니다. |
| 정규화가 줄이는 이상 현상 세 가지는? | 삽입 이상, 수정 이상, 삭제 이상입니다. |
| 역정규화가 적합한 상황은? | 조회가 매우 빈번하고 join 비용이 크거나, read model이 command model과 다르거나, 최신성이 약간 늦어도 되는 화면 등입니다. |
| 역정규화 시 주의할 점은? | 중복 데이터의 source of truth를 정하고, 갱신 실패 시 불일치 복구 방법과 동기/비동기(outbox/eventual consistency) 갱신 방식을 결정해야 합니다. |

### 트랜잭션과 격리 수준

| 질문 | 답변 |
| --- | --- |
| ACID 각 속성의 실패 예시는? | 원자성은 결제됐는데 주문 생성 실패, 일관성은 재고가 음수, 격리성은 같은 쿠폰 중복 사용, 지속성은 commit 후 장애로 데이터 손실입니다. |
| 트랜잭션을 "여러 SQL을 묶는 문법"이라고 보면 안 되는 이유는? | 트랜잭션은 문법이 아니라 비즈니스 불변식을 보호하는 경계이기 때문입니다. |
| 각 격리 수준에서 발생 가능한 현상은? | READ UNCOMMITTED는 dirty/non-repeatable/phantom, READ COMMITTED는 non-repeatable/phantom, REPEATABLE READ는 표준상 phantom, SERIALIZABLE은 성능 저하와 retry 필요 가능입니다. |
| Dirty read, non-repeatable read, phantom read의 차이는? | dirty read는 미커밋 데이터 읽기, non-repeatable read는 같은 row 재조회 시 값이 바뀜, phantom read는 같은 조건 범위 조회에서 row 집합이 바뀌는 현상입니다. |
| PostgreSQL과 MySQL InnoDB의 격리 수준 동작 차이는? | PostgreSQL은 READ UNCOMMITTED를 요청해도 READ COMMITTED처럼 동작하고 REPEATABLE READ가 snapshot isolation 기반이라 phantom을 허용하지 않으며, MySQL InnoDB는 REPEATABLE READ가 기본이고 next-key lock으로 일부 phantom을 막습니다. |
| READ COMMITTED와 REPEATABLE READ의 snapshot 기준 차이는? | READ COMMITTED는 statement마다 새 snapshot을, REPEATABLE READ는 transaction 시작 시점 snapshot을 기준으로 합니다. |
| 격리 수준을 올리면 모든 race condition이 사라지나? | 아닙니다. 격리 수준을 올려도 모든 비즈니스 race condition이 자동으로 사라지지는 않으며, 복잡한 불변식 검증에는 더 강한 격리나 명시적 lock이 필요할 수 있습니다. |
| Lost update와 write skew의 차이와 대응은? | lost update는 같은 값을 읽고 각각 덮어써 한 변경이 사라지는 것으로 row lock/optimistic version/atomic update로, write skew는 서로 다른 row를 읽고 갱신해 전체 제약이 깨지는 것으로 serializable/predicate lock/constraint/explicit lock으로 대응합니다. |
| `UPDATE ... SET stock = stock - 1 WHERE stock > 0` 같은 조건부 update의 이점은? | read 후 update보다 race condition을 줄이기 쉽습니다. |

### Lock과 MVCC

| 질문 | 답변 |
| --- | --- |
| Shared lock과 exclusive lock의 차이는? | shared lock은 읽기 락으로 다른 shared lock과 공존 가능하고, exclusive lock은 쓰기 락으로 다른 어떤 lock과도 공존 불가입니다. |
| 인덱스가 lock 범위에 영향을 주는 이유는? | lock 범위는 query 조건과 index 사용 여부에 따라 커지며, 적절한 index가 없으면 의도보다 많은 row나 range를 잠글 수 있습니다. |
| 낙관적 락과 비관적 락의 선택 기준은? | 읽기 많고 충돌이 낮으면 낙관적 락이, 좌석 예약/재고 차감처럼 충돌이 높고 실패 비용이 크면 비관적 락이나 조건부 update가 유리합니다. |
| MVCC의 핵심 장점은? | 여러 버전을 유지해 읽기가 쓰기를, 쓰기가 읽기를 덜 막고, long-running read도 자신의 snapshot을 유지할 수 있습니다. |
| MVCC가 있어도 쓰기-쓰기 충돌은 어떻게 다루나? | MVCC로 자동 해결되지 않으므로 lock, retry, abort로 다뤄야 합니다. |
| 오래 열린 transaction이 MVCC에서 위험한 이유는? | 이전 version 정리를 막아 undo/vacuum 부담을 키우기 때문입니다. |
| Record lock, gap lock, next-key lock의 차이는? | record lock은 index record 자체를, gap lock은 record 사이 빈 공간을, next-key lock은 record lock과 gap lock을 합친 것을 잠급니다. |
| Gap/next-key lock의 목적은? | 범위 조건에서 phantom insert를 막고, `SELECT ... FOR UPDATE` 범위 조회에서 다른 transaction의 삽입을 막습니다. |
| Lock wait와 deadlock의 차이와 대응은? | lock wait는 다른 transaction의 lock 해제를 기다리는 것으로 timeout/query·index 개선/transaction 축소로, deadlock은 서로의 lock을 기다리며 순환 대기하는 것으로 한쪽 rollback과 lock 순서 통일로 대응합니다. |
| Deadlock 조사 시 운영에서 확인할 것은? | 어떤 SQL이 lock을 잡았는지, 어떤 transaction이 오래 열려 있는지, index 부재로 scan/lock 범위가 커졌는지, transaction 안에 외부 API 호출이 있는지 확인합니다. |

### 인덱스 설계

| 질문 | 답변 |
| --- | --- |
| DB 인덱스가 B+Tree를 쓰는 이유는? | 값을 리프 노드 중심으로 저장하고 리프를 연결해 범위 검색에 유리하며 equality, range, order by에 잘 대응하기 때문입니다. |
| Covering index란? | query에 필요한 column이 모두 index에 있어 table row 접근을 줄이는 index입니다. |
| MySQL InnoDB secondary index 비용이 primary key 크기에 영향을 받는 이유는? | InnoDB secondary index는 primary key 값을 함께 저장하기 때문입니다. |
| 복합 인덱스 column 순서 설계 기준은? | 자주 함께 쓰는 where/order by 패턴을 보고 동등 조건 column을 앞에 두며, range 조건 이후 column은 활용이 제한되므로 정렬 방향과 covering 여부까지 고려합니다. |
| Cardinality와 selectivity란? | cardinality는 column 값의 다양성, selectivity는 조건으로 걸러지는 비율입니다. |
| 통계 정보가 오래되면 생기는 문제는? | optimizer가 잘못된 plan을 고를 수 있습니다. |
| 인덱스가 안 타는 흔한 경우와 대안은? | indexed column에 함수 적용(→ expression index/전처리), leading wildcard `LIKE '%abc'`(→ full-text/검색엔진), type mismatch(→ 타입 맞춤), 부정 조건이나 낮은 선택도(→ 조건 재설계/복합 index) 등입니다. |
| 인덱스가 쓰기 성능을 낮추는 이유는? | insert/update/delete 시 index도 함께 갱신해야 하고 저장 공간도 늘어나기 때문입니다. |

### 실행 계획과 쿼리 튜닝

| 질문 | 답변 |
| --- | --- |
| MySQL EXPLAIN에서 봐야 할 핵심 항목은? | `type`(ALL이면 full scan), `key`(실제 선택 index), `rows`(예상 스캔 row), `filtered`(조건 후 남을 비율), `Extra`(Using filesort/temporary/index)입니다. |
| PostgreSQL EXPLAIN의 Index Scan과 Index Only Scan 차이는? | Index Scan은 index로 찾고 table row에 접근하지만, Index Only Scan은 index만으로 결과를 반환할 수 있습니다. |
| EXPLAIN과 EXPLAIN ANALYZE의 차이는? | EXPLAIN은 예상 계획만 보여주고, EXPLAIN ANALYZE는 실제 실행까지 하며 실제 row와 time을 보여줍니다. |
| EXPLAIN ANALYZE 사용 시 주의점은? | 실제 query를 실행하므로 INSERT/UPDATE/DELETE에 쓰면 실제 변경이 일어날 수 있어 transaction rollback 같은 안전장치가 필요합니다. |
| 예상 row와 실제 row 차이가 크면 무엇을 의심하나? | 통계 정보, 조건 상관관계, parameter 분포를 의심합니다. |
| 느린 쿼리 개선 순서는? | 목적과 반환 row 확인 → EXPLAIN으로 scan/join/sort 확인 → 관련 column index 확인 → projection 축소 → offset paging/N+1/불필요 join 제거 → 통계 갱신 → 그래도 어려우면 schema/denormalization/read model 검토입니다. |
| Nested loop, hash join, merge join이 각각 유리한 상황은? | nested loop는 outer가 작고 inner index가 좋을 때, hash join은 대량 equi-join, merge join은 양쪽이 정렬돼 있거나 range join일 때입니다. |
| Join 성능을 좌우하는 요소는? | 단순 join 수가 아니라 join 순서, cardinality 추정, access path, 중간 결과 크기입니다. |

### 페이징과 데이터 모델링

| 질문 | 답변 |
| --- | --- |
| Offset paging이 느린 이유는? | 뒤 페이지로 갈수록 건너뛸 row가 많아져 latency가 증가하고, 중간에 insert/delete가 일어나면 중복/누락이 생길 수 있습니다. |
| Cursor(keyset) paging의 장점은? | 뒤 페이지에서도 안정적이고 index를 활용하기 쉬워 무한 스크롤, feed, log 조회에 적합합니다. |
| Cursor paging의 단점은? | 임의 페이지 이동이 어렵고, 정렬 기준이 unique·안정적이어야 하며, 복합 정렬이면 cursor 조건도 복합으로 만들어야 합니다. |
| 논리 삭제의 대표적 문제는? | 조회 조건 누락, table/index 비대화, 그리고 모든 unique constraint가 논리 삭제 row와 충돌할 수 있는 점입니다. |
| 논리 삭제에서 unique 정책은 어떻게 명확히 하나? | partial index 또는 composite unique key로 정책을 명확히 하고, 오래된 삭제 row 정리 정책을 둡니다. |
| 금액 계산에 FLOAT/DOUBLE이 부적합한 이유는? | 근사값이라 정확한 10진 계산에 부적합하며, 금액에는 DECIMAL이 적합합니다. |
| Random UUID를 primary key로 쓸 때 주의점은? | 분산 생성은 쉽지만 index locality에 불리해 B+Tree page split과 index size 증가를 유발합니다. |

### 확장과 운영

| 질문 | 답변 |
| --- | --- |
| Statement-based와 row-based replication의 차이는? | statement-based는 SQL 문장을 기록해 작지만 비결정적 SQL에 취약하고, row-based는 변경된 row를 기록해 정확하지만 log가 커질 수 있습니다. |
| Replication lag가 만드는 문제는? | 쓰기 직후 read replica에서 읽으면 최신 데이터가 안 보일 수 있습니다. |
| Read-your-writes가 필요할 때의 전략은? | primary read, session consistency(stickiness), lag 감지 기반 routing 등이 필요합니다. |
| Partitioning과 sharding의 차이는? | partitioning은 하나의 DB 논리/서버 안에서 table을 나누는 것이고, sharding은 여러 DB 인스턴스에 데이터를 나누는 것입니다. |
| Sharding의 대표적 어려움은? | shard key 선택, cross-shard join, 분산 transaction과 global unique key, resharding 비용, hot shard 발생입니다. |
| Connection pool이 너무 작거나 너무 클 때의 문제는? | 너무 작으면 많은 요청이 connection을 대기하고, DB가 처리 가능한 수보다 크게 잡으면 DB 자체가 병목이 됩니다. |
| Slow query가 connection pool에 미치는 영향은? | connection 점유 시간을 늘려 pool 고갈을 유발합니다. |
| Backup에서 가장 중요한 것은? | backup 자체보다 복구 가능한 backup인지 검증하는 것이며, 주기적 restore drill이 필요합니다. |
| RPO와 RTO의 차이는? | RPO는 어느 시점까지의 데이터 손실을 허용하는가, RTO는 얼마나 빨리 복구해야 하는가입니다. |
| PreparedStatement가 Statement보다 나은 점은? | placeholder binding으로 SQL Injection에 안전하고 plan 재사용이 가능합니다. |

### 심화 설계와 운영 패턴

| 질문 | 답변 |
| --- | --- |
| SARGable query란? | Search ARGument ABLE의 줄임말로, DB가 인덱스 탐색 조건으로 쓸 수 있는 query입니다. `DATE(created_at) = ...` 대신 범위 조건으로 바꾸면 인덱스를 탈 수 있습니다. |
| "인덱스가 있다"와 "인덱스를 효율적으로 탄다"의 차이는? | 인덱스가 존재해도 함수 감싸기, leading wildcard, 암묵적 타입 변환, 넓은 OR 조건 등으로 실제로는 index를 효율적으로 못 탈 수 있습니다. |
| Hot row contention의 대표 예와 증상은? | 재고 row, view count row, 전역 counter, 인기 쿠폰 사용량 row에 갱신이 몰리는 것으로, lock wait/deadlock/transaction latency/connection 점유/replication lag 증가로 나타납니다. |
| Hot row 대응 전략과 트레이드오프는? | 조건부 atomic update(재시도 필요), counter sharding(실시간 합계 비용 증가), queue/serial worker(latency·worker 장애 대응), optimistic lock(retry storm 가능), cache write-behind(데이터 손실/지연 허용) 중 충돌 빈도와 실패 비용에 따라 선택합니다. |
| DB 기반 idempotency 패턴의 핵심은? | client가 보낸 idempotency key를 unique constraint로 저장하고, 같은 key가 다시 오면 기존 결과를 반환하거나 진행 중 상태를 확인하며, 같은 key에 다른 request body가 오면 충돌로 봅니다. |
| Transaction 안에 외부 API 호출을 넣으면 안 되는 이유는? | lock과 connection 점유 시간이 길어지고, 외부 API는 DB rollback과 함께 되돌릴 수 없어 retry와 보상 transaction을 따로 설계해야 하기 때문입니다. |
| Outbox pattern이 해결하는 문제는? | DB commit은 성공했는데 메시지 발행이 실패하는 문제를 줄이며, message broker transaction에 직접 의존하지 않고 eventual consistency를 설계할 수 있게 합니다. |
| Outbox 사용 시 consumer가 갖춰야 할 것은? | 중복 메시지를 처리할 수 있어야 하고, outbox table 정리 정책과 필요 시 발행 순서 설계가 있어야 합니다. |
| Online schema migration의 expand-migrate-contract 순서는? | Expand(호환되게 새 구조 추가) → dual write/read 호환 → Backfill(작은 batch로 기존 데이터 채움) → Cutover(read path 전환) → Contract(안 쓰는 column/index 제거)입니다. |
| Migration 시 큰 table에서 주의할 점은? | index 생성 시 lock/I/O/replication lag를 확인하고, default 값 column 추가가 table rewrite를 유발할 수 있으며, backfill은 작은 batch/sleep/checkpoint/재시작 가능성을 고려하고 rollback용 backward compatibility 기간을 둡니다. |
| 통계와 cardinality 추정이 빗나가는 원인은? | 통계가 오래되거나, 컬럼 간 상관관계가 크거나, skewed data라 평균 selectivity로 설명하기 어려운 경우이며 ANALYZE, extended statistics, query rewrite로 대응합니다. |

### 심화: 스토리지 엔진 내부

| 질문 | 답변 |
| --- | --- |
| DB가 디스크와 메모리를 오가는 최소 단위는? | 행이 아니라 page(InnoDB 16KB, PostgreSQL 8KB)이며, 자주 쓰는 page는 버퍼 풀에 캐싱됩니다. |
| 쓰기가 있어도 즉시 data 파일에 랜덤하게 쓰지 않는 이유는? | 버퍼 풀에서 수정하고 디스크 반영을 뒤로 미뤄 랜덤 I/O를 줄이기 위함입니다. |
| InnoDB의 midpoint insertion LRU가 필요한 이유는? | 큰 스캔 한 번이 자주 쓰는 page를 버퍼 풀에서 밀어내는 것을 막기 위함입니다. |
| WAL의 핵심 원칙은? | data page보다 로그를 먼저 디스크에 쓴다("로그 먼저")는 것으로, 이것이 ACID의 D(내구성)를 만드는 실제 메커니즘입니다. |
| redo log와 undo log의 역할 차이는? | redo log(WAL)는 커밋됐지만 data page에 반영 안 된 변경을 crash 복구 시 재적용하고, undo log는 미커밋 변경 롤백과 MVCC 이전 버전 제공을 담당합니다. |
| WAL이 커밋 내구성 비용을 낮추는 방식은? | 랜덤한 data page write 대신 작은 순차 로그 fsync로 치환하며, group commit으로 여러 트랜잭션의 fsync를 묶어 I/O 횟수를 줄입니다. |
| Checkpoint가 복구를 빠르게 하는 원리는? | dirty page를 디스크에 반영하고 안전 지점을 기록해, 복구 시 전체 로그가 아니라 마지막 checkpoint 이후 redo log만 재적용하고 미커밋은 undo로 롤백하기 때문입니다. |
| InnoDB clustered index와 PostgreSQL heap의 저장 차이는? | InnoDB는 PK B+Tree leaf에 행 전체를 저장하고 secondary index leaf에 PK 값을 넣으며, PostgreSQL은 행을 heap에 저장하고 모든 인덱스가 물리 위치(ctid)를 가리킵니다. |
| InnoDB secondary index 조회가 2단계인 이유와 튜닝법은? | secondary index leaf에서 PK를 얻고 다시 clustered index를 타기 때문이며, 필요 컬럼을 인덱스에 포함하는 covering index로 2차 조회를 없앱니다. |
| InnoDB에서 단조 증가 PK가 유리하고 랜덤 UUID가 불리한 이유는? | PK 순서로 물리 정렬되므로 auto_increment는 유리하고, 랜덤 UUID PK는 page split과 write amplification을 유발합니다. |
| LSM-Tree와 B+Tree의 쓰기 방식 차이는? | B+Tree는 data page를 제자리에서 갱신(랜덤 write)+WAL을 쓰고, LSM-Tree는 memtable에 순차 축적 후 SSTable로 flush하고 background compaction으로 병합합니다. |
| LSM-Tree와 B+Tree는 각각 어떤 워크로드에 적합한가? | 쓰기 폭주+시계열/로그성 데이터면 LSM 계열, 범용 트랜잭션+복잡한 조회면 B+Tree 계열이 일반적 선택이며, LSM은 compaction이 write amplification을 만듭니다. |
...
- (커뮤니티 면접 정리) Backend Interview for Beginner — Database: https://github.com/backtony/Backend_Interview_for_Beginner/blob/master/Database.md
