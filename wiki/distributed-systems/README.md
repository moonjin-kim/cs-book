...
단일 process 내부에서는 함수 호출이 성공하거나 예외를 던져 결과가 비교적 명확합니다. 분산 환경에서는 요청을 보낸 쪽이 다음을 구분하기 어렵습니다.
...
대부분의 백엔드 면접에서 다루는 Raft, Paxos, quorum replication은 crash failure와 network partition을 주로 가정하며, Byzantine fault tolerant는 별도 영역입니다.
...
현실의 분산 시스템은 partition을 완전히 배제할 수 없으므로, 그 상황에서 C와 A 중 무엇을 희생할지 고민합니다.
...
Cascading failure는 한 컴포넌트 장애가 retry, thread 고갈, connection 고갈, queue 증가를 통해 다른 컴포넌트로 번지는 현상입니다.
