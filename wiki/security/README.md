...
- Gateway 계층에서 인증을 일원화하는 패턴이 있습니다. 앱이 유저 식별키로 요청하면 Gateway가 인증 서버에서 디바이스와 유저 정보를 담은 "Passport"를 발급받아 serialize해 하위 서비스로 전파하므로, 각 서비스가 유저 API를 개별 호출할 필요가 없어집니다(Netflix Passport 구조 참고). (출처: 토스)
- Passport용 유저 정보는 짧은 TTL로 캐시하고 Redis pub/sub으로 변경 이벤트 시 expire합니다 — 캐시 성능과 claim 최신성의 트레이드오프를 짧은 TTL과 무효화 이벤트로 절충한 사례입니다. (출처: 토스)
