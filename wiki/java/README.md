...
전체를 한 페이지에 담되, 우측 목차로 필요한 섹션만 골라 읽습니다.
...
| 9 | 실전 면접 Q&A | 짧은 답변으로 핵심 질문을 빠르게 복습할 수 있는가? |
...
- non-LTS는 다음 release가 나오면 곧 대체되므로 장기 운영 기준선으로 삼기 어렵습니다.
...
클래스는 처음에 전부 로딩되지 않고, 실제 사용 시점에 로딩됩니다.
...
JVM은 bytecode를 기계어로 고정하지 않고, 실행 중 수집한 정보로 최적화합니다.
...
- cache는 `SoftReference`보다 Caffeine 같은 라이브러리로 크기, TTL, 통계를 명시적으로 관리하는 편이 실무적입니다.
- `finalize()`는 예측 가능성이 낮고 deprecated 되었으므로 `try-with-resources`, `Cleaner`, 명시적 close를 우선합니다.

### 메모리 누수와 OOM

GC가 있어도 **도달 가능한 객체**는 회수되지 않아, Java에도 메모리 누수가 생깁니다.
...
`HashMap`은 key의 hash로 bucket을 찾고, 같은 bucket 안에서 key 동등성을 확인합니다.
...
Virtual thread는 OS thread를 직접 점유하는 platform thread보다 가벼워, 많은 동시 작업을 적은 비용으로 다루는 `Thread` 구현입니다.
...
Structured Concurrency는 여러 자식 작업의 성공·실패·취소·대기를 하나의 작업 단위로 다루려는 모델입니다.
...
JIT는 실행 중 수집한 profile로 hot code를 native code로 바꿔 최적화합니다. 그래서 Java 성능은 “컴파일된 코드”만 봐선 부족하고, **warm-up, profile, deoptimization**까지 함께 봐야 합니다.
