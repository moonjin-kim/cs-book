...
| 질문 | 답변 |
| --- | --- |
| JVM이 뭔가? | Java 코드를 실제로 실행해 주는 프로그램입니다. 우리가 짠 코드를 컴퓨터가 이해하는 명령으로 바꿔 돌려주고, 메모리 관리도 대신 해 줍니다. |
| 컴파일과 인터프리트(해석)의 차이는? | 컴파일은 코드를 미리 한꺼번에 다른 형태로 번역해 두는 것이고, 인터프리트는 실행할 때 한 줄씩 그때그때 해석하는 것입니다. Java는 먼저 `.class`로 컴파일한 뒤 실행 시 한 줄씩 해석하다가 자주 쓰는 부분만 빠른 기계어로 바꿉니다. |
| 가비지 컬렉션(GC)이 뭔가? | 더 이상 쓰지 않는 객체가 차지한 메모리를 JVM이 알아서 찾아 치워 주는 기능입니다. 덕분에 개발자가 메모리를 직접 비우지 않아도 됩니다. |
| GC는 어떤 객체를 치우나? | 어디에서도 더 이상 참조(사용)하지 않는 객체를 치웁니다. 아직 누군가 붙잡고 쓰는 객체는 남겨 둡니다. |
| Heap과 Stack의 차이는? | Heap은 `new`로 만든 객체가 저장되는 공용 공간이고, Stack은 메서드 안의 지역 변수처럼 잠깐 쓰는 값이 저장되는 스레드별 공간입니다. |
| `==` 과 `equals()`의 차이는? | `==`는 두 변수가 같은 객체(같은 메모리 위치)를 가리키는지 보고, `equals()`는 값이 같은지를 봅니다. 값이 같은지 확인하려면 `equals()`를 씁니다. |
| String이 "불변"이라는 게 무슨 뜻인가? | 한번 만든 문자열은 내용을 바꿀 수 없다는 뜻입니다. 문자열을 바꾸는 것처럼 보여도 실제로는 새 문자열 객체가 만들어집니다. |
| 반복문에서 문자열을 `+`로 계속 이으면 뭐가 문제인가? | String은 불변이라 이을 때마다 새 문자열 객체가 계속 생겨 낭비가 됩니다. 이럴 땐 `StringBuilder`를 쓰면 좋습니다. |
| List, Set, Map은 각각 뭔가? | List는 순서가 있고 중복을 허용하는 목록, Set은 중복을 허용하지 않는 모음, Map은 키와 값을 짝지어 저장하는 사전 같은 구조입니다. |
| `int`(primitive)와 `Integer`(wrapper)의 차이는? | `int`는 숫자 값 자체를 담는 기본 타입이고, `Integer`는 그 숫자를 감싼 객체입니다. `Integer`는 값이 없음(null)을 표현할 수 있지만 `int`는 못 합니다. |
| final을 붙이면 어떻게 되나? | 변수에 붙이면 값을 한 번만 정하고 다시 바꿀 수 없고, 메서드에 붙이면 자식이 다시 정의(override)할 수 없으며, 클래스에 붙이면 상속할 수 없습니다. |
| static이 뭔가? | 특정 객체 하나가 아니라 클래스 전체에 속하는 것입니다. 그래서 객체를 만들지 않아도 쓸 수 있고, 모든 객체가 같은 값을 공유합니다. |
