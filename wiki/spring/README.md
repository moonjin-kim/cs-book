...
| 질문 | 답변 |
| --- | --- |
| Spring이 무엇인가? | 자바로 서버 프로그램을 만들 때 반복되는 일(객체 만들기, 연결하기, 트랜잭션 처리 등)을 대신 관리해 주는 프레임워크입니다. 개발자는 핵심 로직에 더 집중할 수 있습니다. |
| 컨테이너(IoC 컨테이너)가 무엇인가? | 우리가 만든 객체들을 대신 만들고, 서로 연결해 주고, 수명까지 관리해 주는 Spring의 관리자 역할입니다. |
| IoC가 무엇인가? | 객체를 언제 만들고 어떻게 연결할지를 내 코드가 아니라 컨테이너가 대신 결정하게 맡기는 것입니다. "제어를 넘긴다"는 뜻입니다. |
| DI(의존성 주입)가 무엇인가? | 어떤 객체가 필요로 하는 다른 객체를 스스로 만들지 않고, 외부(컨테이너)에서 받아 쓰는 방식입니다. 예를 들어 요리사가 재료를 직접 사오지 않고 배달받는 것과 비슷합니다. |
| IoC와 DI는 어떻게 다른가? | IoC는 "제어를 컨테이너에 넘긴다"는 큰 개념이고, DI는 그것을 실제로 이루는 방법(필요한 객체를 외부에서 넣어주기)입니다. |
| 왜 DI를 쓰는가? | 객체가 다른 객체를 직접 만들지 않으니 서로 덜 얽히고(결합도가 낮고), 나중에 바꾸거나 테스트하기 쉬워집니다. |
| Bean이 무엇인가? | Spring 컨테이너가 대신 만들고 관리해 주는 객체입니다. 컨테이너에 등록된 객체라고 보면 됩니다. |
| 왜 객체를 Bean으로 관리하는가? | 컨테이너가 객체 생성과 연결을 대신해 주고, 트랜잭션 같은 공통 기능을 붙여주며, 테스트할 때 가짜 객체로 바꾸기도 쉬워지기 때문입니다. |
| AOP가 무엇인가? | 로그 남기기, 트랜잭션, 보안 검사처럼 여러 곳에서 똑같이 반복되는 공통 기능을 한 곳에 모아 두고, 메서드 실행 앞뒤에 자동으로 끼워 넣는 방식입니다. |
| `@Transactional`이 무엇인가? | 메서드가 하는 DB 작업을 하나로 묶어서, 중간에 문제가 생기면 전부 취소하고 성공하면 한꺼번에 저장하도록 해주는 표시입니다. |
| JPA가 무엇인가? | 자바 객체와 데이터베이스 테이블을 자동으로 연결해 주는 표준 규칙입니다. SQL을 직접 많이 쓰지 않고도 객체를 다루듯 데이터를 저장·조회할 수 있게 해줍니다. |
| Spring과 Spring Boot는 어떻게 다른가? | Spring은 설정을 직접 많이 해야 하지만, Spring Boot는 그 설정을 자동으로 해주고 서버까지 안에 품고 있어 훨씬 쉽게 바로 실행할 수 있게 해줍니다. |
