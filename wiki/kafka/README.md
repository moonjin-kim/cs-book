...
| 질문 | 답변 |
| --- | --- |
| Kafka는 한마디로 무엇인가? | 여러 시스템 사이에서 오가는 데이터(이벤트)를 실시간으로 주고받게 해주는 프로그램입니다. 보낸 데이터를 로그처럼 차곡차곡 쌓아두고, 받는 쪽이 필요할 때 읽어갑니다. |
| 이벤트(메시지)란 무엇인가? | "주문이 들어왔다", "결제가 완료됐다"처럼 시스템에서 일어난 하나의 사건을 담은 데이터 조각입니다. Kafka는 이런 이벤트를 실어 나릅니다. |
| 프로듀서와 컨슈머는 각각 뭘 하나? | 프로듀서(Producer)는 Kafka에 메시지를 보내는(쓰는) 쪽이고, 컨슈머(Consumer)는 Kafka에서 메시지를 받아(읽어) 처리하는 쪽입니다. |
| 브로커(Broker)가 뭔가? | 메시지를 실제로 저장하고 보관하는 Kafka 서버 한 대를 말합니다. 프로듀서가 보낸 메시지를 받아 저장하고, 컨슈머의 요청에 맞춰 꺼내줍니다. |
| 토픽(Topic)이 뭔가? | 메시지를 종류별로 나눠 담는 상자(이름표)입니다. 예를 들어 "주문" 토픽, "결제" 토픽처럼 나눠서 관련 있는 메시지끼리 모아둡니다. |
| 파티션(Partition)이 뭔가? | 하나의 토픽을 여러 조각으로 쪼갠 것입니다. 메시지를 여러 조각에 나눠 담으면 여러 컨슈머가 동시에 나눠 읽을 수 있어 더 빠르게 처리됩니다. |
| 토픽과 파티션의 차이는? | 토픽은 "메시지를 담는 논리적인 이름"이고, 파티션은 그 토픽을 실제로 저장하고 나눠 처리하는 물리적인 단위입니다. 하나의 토픽은 여러 파티션으로 이루어집니다. |
| 오프셋(Offset)이 뭔가? | 파티션 안에서 메시지에 순서대로 붙는 번호(위치 표시)입니다. 컨슈머는 "몇 번까지 읽었다"를 오프셋으로 기억해서, 다음에 이어서 읽을 위치를 압니다. |
| 컨슈머 그룹(Consumer Group)은 왜 필요한가? | 여러 컨슈머를 한 팀으로 묶어 파티션을 나눠 맡기기 위해서입니다. 일을 여러 명이 분담하듯 메시지를 나눠 처리해 더 빠르게 소화할 수 있습니다. |
| 같은 파티션을 한 그룹 안에서 여러 컨슈머가 나눠 읽을 수 있나? | 아니요. 한 파티션은 같은 그룹 안에서 한 컨슈머만 읽습니다. 그래서 컨슈머를 아무리 늘려도 파티션 수보다 많으면 남는 컨슈머는 놀게 됩니다. |
| Kafka는 메시지를 읽으면 바로 지우나? | 아니요. 컨슈머가 읽어도 곧바로 지우지 않고 정해진 보관 기간(retention) 동안 그대로 둡니다. 그래서 나중에 다시 읽거나 다른 컨슈머가 또 읽을 수 있습니다. |
| Kafka와 일반 메시지 큐(예: RabbitMQ)의 가장 큰 차이는? | 일반 큐는 메시지를 꺼내 처리하면 사라지지만, Kafka는 로그처럼 쌓아두고 읽어도 남겨둡니다. 그래서 나중에 다시 읽거나 되돌려 처리하기가 쉽습니다. |
