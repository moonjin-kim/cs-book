...
- `LinkedList`는 삽입/삭제가 항상 빠르지 않습니다. 삽입 위치를 찾는 비용이 듭니다.
- 단순 목록 처리에서는 `ArrayList`가 캐시 효율 때문에 더 유리한 경우가 많습니다.
...
- 충돌이 몰리면 O(n)이 될 수 있어 충돌 대응 전략이 필요합니다.
...
Heap은 전체를 정렬하지 않고, root에 최우선 원소가 오도록 유지합니다.
