...
코드에 존재하는 비즈니스 개념과 그 관계를 기록하여, Claude가 코드를 빠르게 찾아가도록 돕는 **내비게이션 시스템**이다.

각 entity는 `repo` + `package` 필드로 코드 위치가 고정되어 있다. Claude는 ontology를 읽고 "이 개념은 이 레포의 이 패키지에 있다"를 즉시 파악한다. 이것이 ontology의 핵심 가치다.

### 이것이 아닌 것

- **디지털 트윈이 아니다.** 팔란티어 온톨로지처럼 비즈니스 운영 전체를 모델링하지 않는다. 팔란티어는 Object Storage + OSDK라는 런타임 동기화 시스템으로 온톨로지와 데이터를 자동 연결한다. CC에는 그런 자동화가 없다.
- **지식 그래프가 아니다.** RDF/OWL이나 Neo4j처럼 쿼리하는 데이터베이스가 아니다. YAML 파일을 Claude가 읽는 것이 전부다.
- **비즈니스 도메인 모델이 아니다.** 코드에 없는 개념(수동 워크플로, 조직 구조, 마케터의 엑셀 전달 과정 등)은 여기에 넣지 않는다.

### 왜 코드 중심인가

CC 온톨로지는 사람과 Claude가 수동으로 유지보수한다. 자동 동기화 파이프라인이 없으므로, **코드에서 존재를 확인할 수 있는 것만** 기록해야 정합성이 유지된다.

- `repo` + `package`가 가리키는 코드가 실존하는지는 검증 가능하다
- 코드가 변경되면 ontology도 갱신해야 하는데, 코드 앵커가 있어야 "이 entity가 아직 유효한가?"를 판단할 수 있다
- 코드 앵커가 없는 entity는 시간이 지나면 현실과 괴리되고, 검증할 방법도 없다

### 비즈니스 맥락은 wiki에

코드에 없는 비즈니스 정책, 의사결정 배경, 워크플로 시나리오는 `wiki/`에서 다룬다. 3계층을 분리한 이유가 이것이다:

| 계층 | 역할 | 정합성 앵커 |
|------|------|-------------|
| **ontology** | 코드에 있는 것의 지도 | `repo` + `package` (코드 존재 확인) |
| **wiki** | 코드 밖 비즈니스 맥락 포함 | 사람이 작성·검토 |
| **code** | 실제 구현 (진실의 원천) | — |

ontology entity의 `wiki_doc` 필드가 두 계층을 연결한다. "이 코드가 뭐하는지"는 ontology가, "왜 이렇게 만들었는지"는 wiki가 답한다.
...
설계 철학은 이 문서 상단의 "설계 철학" 섹션을 참조한다.
...
- 미채택 유형: caches, fallback_to, mirrored_to, determines, transforms, bypasses, shared_schema, incentivizes
