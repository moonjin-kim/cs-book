...
- clone 명령:
  ...
  ```
- clone한 레포가 `workspace.json`에 없으면 추가할지 물어보고, 승인 시 간단한 설명(desc)을 입력받아 `projects.optional` 배열에 `{ "repo": "...", "desc": "..." }` 형태로 Edit 도구를 사용해 추가한다. `ghe_host`와 다른 호스트에서 clone한 경우 `"host": "{host}"` 필드도 함께 추가한다.
